...
export async function runScrollLoop(opts: {
  ...
  const maxUsers = opts.maxUsers ?? 10000;
  ...
  const observer = new MutationObserver((mutations) => {
    for (const m of mutations) {
      for (const node of Array.from(m.addedNodes)) {
        ...
        if (
          node.matches?.('[data-testid="UserCell"]') ||
  ...
  try {
    ...
    while (idleRounds < maxIdleRounds && rounds < hardCapRounds) {
      ...
      if (preCount >= maxUsers) {
        opts.onTick({ rounds, idleRounds, visibleCells: preCount, progressed: false, delayMs: settleMs, elapsedMs });
      ...
      settleMs = progressed
        ? clamp(settleMs - 60, minDelay, maxDelay)
        : clamp(settleMs + 120, minDelay, maxDelay);
      ...
      opts.onTick({ rounds, idleRounds, visibleCells: currentCount, progressed, delayMs: settleMs, elapsedMs });
    }
